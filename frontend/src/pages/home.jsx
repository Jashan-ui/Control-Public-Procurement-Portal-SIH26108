                ...
                <button
                  ...
                  className={`w-full text-left text-xs px-3 py-2 rounded-lg transition-colors truncate ${
                    activeChatId === chat.id
                      ? 'bg-[#a855f7]/15 text-[#c4b5fd] border border-[#a855f7]/30'
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          {messages.length === 1 && (
            <div className="px-6 pt-10 pb-4 max-w-4xl mx-auto w-full text-center">
              <h1 className="font-serif-title text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-[#a855f7] via-white to-[#8b5cf6] mb-3 drop-shadow-md">
              ...
              <div className="flex flex-wrap justify-center gap-2">
                {standardBadges.map((badge, idx) => (
                  <button
                    ...
                    className="px-3.5 py-1.5 rounded-full glass-panel hover:border-[#a855f7]/60 text-xs text-white/80 hover:text-[#c4b5fd] transition-all"
                  >
                    {badge}
                  </button>
                ))}
            ...
            {messages.map((msg, index) => (
              <div key={index} className={`flex gap-3 items-start ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                <div className={`max-w-[85%] rounded-2xl p-4 text-sm leading-relaxed shadow-xl ${
                  msg.role === 'user'
                    ? 'bg-gradient-to-r from-[#a855f7] to-[#7e22ce] text-white rounded-tr-none shadow-purple-900/30'
                    : 'glass-panel text-[#ede9fe] rounded-tl-none message-body'
                }`}>
                  {msg.role === 'assistant' ? (
                ...
                {msg.role === 'user' && (
                  <div className="w-8 h-8 rounded-xl bg-gradient-to-tr from-[#a855f7] to-[#7e22ce] flex items-center justify-center shrink-0 shadow-lg">
            ...
            {loading && messages[messages.length - 1].content === '' && (
              <div className="flex gap-3 items-start">
                ...
                <div className="glass-panel rounded-2xl p-4 text-sm flex items-center gap-2">
                  <span className="w-2 h-2 bg-[#a855f7] rounded-full animate-bounce"></span>
                  <span className="w-2 h-2 bg-white rounded-full animate-bounce [animation-delay:0.2s]"></span>
                  <span className="w-2 h-2 bg-[#8b5cf6] rounded-full animate-bounce [animation-delay:0.4s]"></span>
                </div>
              </div>
            )}
          </div>

          <footer className="p-4 border-t border-white/10 bg-[#0a0a0a]/70 backdrop-blur-md shrink-0">
            <div className="max-w-3xl mx-auto">
              {activeFile && (
                <div className="mb-2 flex items-center gap-2 bg-[#1a1a1a] border border-white/15 text-white/80 px-3 py-1.5 rounded-xl text-xs w-fit">
                  <span>{activeFile.name}</span>
                  <button onClick={removeFile} className="ml-2 hover:text-red-400 font-bold">×</button>
                </div>
              )}

              <div className="relative flex items-center bg-[#141414] border border-white/15 focus-within:border-white/40 rounded-2xl p-2 transition-all">
                <input
                  type="text"
                  value={inputQuery}
                ...
                />
                <button onClick={submitQuery} className="bg-[#2b2b2b] hover:bg-[#383838] border border-white/10 text-white px-4 py-2.5 rounded-xl font-medium text-sm transition-all flex items-center gap-1.5">
                  <span>Send</span>
